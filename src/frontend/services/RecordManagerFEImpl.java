...
{
	/**
	 * Instance of FIFOOrderSys for invoking FIFO total request ordering
	 * methods.
	 */
	...
	/**
	 * Object used as a lock for lead server UDP connection details update and
	 * access.
	 */
	 ...
	 * 
	 * @param leadServerDetails
	 *            String containing the leader hostname and port number
	 */
	...
	/**
	 * Communicates a client request to create a new teacher record to the lead
	 * server via the FIFO System.
	 * 
	 * @param mgrID
	 *            Unique ID of the center manager who performs this operation
	 * @param firstName
	 ...
	 *            Subject that the teacher specializes in (e.g. French, Math,
	 *            etc.)
	 * @param location
	 *            Location of the teacher (e.g. MTL, LVL, etc.)
	...
	public String createTRecord(String mgrID, String firstName, String lastName, String address, String phone,
		...
		methodArgs.add(location);
			...
			try {
				opStatus = processRequest(methodName, methodArgs);
			} catch (IOException e) {
	...
	/**
	 * Communicates a client request to create a new student record to the lead
	 * server via the FIFO System.
	 * 
	 * @param mgrID
	 *            Unique ID of the center manager who performs this operation
	 ...
	 *            List of courses that the student has registered for (e.g.
	 *            French, Math, etc.)
	 * @param status
	 *            Status of the student (active/inactive)
	 * @param statusDate
	 *            Date of last status update of the student
	 * @return Success or failure status message of the operation
	 */
	...
	/**
	 * Communicates a client request to fetch server record counts to the lead
	 * server via the FIFO System.
	 * 
	 * @param mgrID
	 *            Unique ID of the center manager who performs this operation
	 * @return Record counts of all center servers, if successful Failure status
	 *         message, otherwise
	 */
	public String getRecordCounts(String mgrID) {
		...
		try {
			recCount = processRequest(methodName, methodArgs);
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	...
	/**
	 * Communicates a client request to edit an existing record to the lead
	 * server via the FIFO System.
	 * 
	 * @param mgrID
	 *            Unique ID of the center manager who performs this operation
	 * @param recordID
	 *            Unique ID for identifying the record to be updated
	 * @param fieldName
	 *            Name of the field in the record to be updated
	 * @param newValue
	 *            New value to be assigned to the field
	 * @return Success or failure status message of the operation
	 */
	...
	/**
	 * Communicates a client request to transfer a record to another Center
	 * Server to the lead server via the FIFO System.
	 * 
	 * @param mgrID
	 *            Unique ID of the center manager who performs this operation
	 * @param recordID
	 *            Unique ID for identifying the record to be transferred
	 * @param remServerName
	 *            Acronym for identifying the destination server
	 * @return Success or failure status message of the operation
	 */
	...
	public String crashLeadServer() {
		...
		// If UDP server is required, the same host and port used by FIFO should
		// be used
	...
	/**
	 * Invokes FIFO System for adding a new Request to the queue and getting it
	 * processed by the lead server.
	 * 
	 * @param methodName
	 *            Name of the method to be invoked on the server
	 * @param methodArgs
	 *            Arguments required as input to the server method
	 * @return Success or failure status message of the processing
	 * @throws IOException
	 */
	private String processRequest(String methodName, List<String> methodArgs) throws IOException {
		String processStatus = null;

		// Creating a Request object with details of the client request to be
		// processed
		...
		if (!reqAdded)
			...
			// Invoking FIFO System to send first request in the queue to the
			// lead server for processing
			processStatus = fifoOrdSys.sendFirstRequest(leadHostname, leadPort);

			// Invoking FIFO System to remove the request just processed from
			// the queue
